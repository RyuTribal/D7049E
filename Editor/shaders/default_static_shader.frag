...
void main() {
    ...
    vec3 emissiveColor = texture(u_EmissiveTexture, texCoords).rgb;
    vec3 emissiveLinear = sRGBToLinear(emissiveColor);
    vec3 emissive = emissiveLinear * 1.0;
...
vec3 CalcPointLight(PointLightInfo light, vec3 N, vec3 V, vec3 F0, float roughness, float metalness, vec3 albedo, vec3 P) {
    ...
    vec3 specularColor = texture(u_SpecularTexture, texCoords).rgb;
    vec3 effectiveF0 = mix(F0, specularColor, specularColor);

    float NDF = DistributionGGX(max(dot(N, H), 0.0), roughness);
    float G = GeometrySmith(max(dot(N, V), 0.0), max(dot(N, L), 0.0), roughness);
    vec3 F = FresnelSchlick(max(dot(H, V), 0.0), effectiveF0);
